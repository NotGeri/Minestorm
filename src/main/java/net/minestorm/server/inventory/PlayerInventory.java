package net.minestorm.server.inventory;
...
import net.minestorm.server.inventory.click.InventoryClickResult;
import net.minestorm.server.item.ItemStack;
import net.minestorm.server.network.packet.server.play.SetSlotPacket;
import net.minestorm.server.network.packet.server.play.WindowItemsPacket;
...
public class PlayerInventory extends AbstractInventory implements EquipmentHandler {
    ...
    public @NotNull ItemStack getItemInOffHand() {
        return getItemStack(PlayerInventoryUtils.OFFHAND_SLOT);
    }

    @Override
    public void setItemInOffHand(@NotNull ItemStack itemStack) {
        safeItemInsert(PlayerInventoryUtils.OFFHAND_SLOT, itemStack);
    }
    ...
    protected void UNSAFE_itemInsert(int slot, @NotNull ItemStack itemStack) {
        ...
        } else if (slot == PlayerInventoryUtils.OFFHAND_SLOT) {
            equipmentSlot = EquipmentSlot.OFF_HAND;
        } else if (slot == PlayerInventoryUtils.HELMET_SLOT) {
        ...
        // Refresh slot
        sendSlotRefresh((short) PlayerInventoryUtils.convertToPacketSlot(slot), itemStack);
    }
    ...
    public boolean shiftClick(@NotNull Player player, int slot) {
        ...
        final ItemStack cursor = getCursorItem();
        final ItemStack clicked = getItemStack(convertedSlot);
        final boolean hotBarClick = PlayerInventoryUtils.convertSlot(slot, PlayerInventoryUtils.OFFSET) < 9;
    ...
    public boolean dragging(@NotNull Player player, int slot, int button) {
        ...
        final InventoryClickResult clickResult = clickProcessor.dragging(player, this,
                PlayerInventoryUtils.convertPlayerInventorySlot(slot, PlayerInventoryUtils.OFFSET), button, clicked, cursor);
